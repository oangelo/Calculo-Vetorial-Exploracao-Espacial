...
- **Tarefa de montar ≠ fornecer pronto:** se a questão pede ao aluno montar/escrever/derivar algo (transformação, matriz jacobiana, expressão, fórmula), NENHUMA parte disso pode aparecer pronta no arquivo — nem no enunciado, nem no diálogo, nem em dica, nem em outro subitem. Entradas a preencher dentro de equações LaTeX usam `\rule{8mm}{0.5pt}` (ex.: matriz com entradas em branco; menores $2\times2$ de expansão em branco). Partes da resposta também não podem ser citadas (ex.: multiplicador de cofator, linha de expansão com valores).
- **Checkpoint é verificação genérica:** faz sentido SEM revelar a resposta (padrão cap-0/1/2). NUNCA dar resposta numérica exata no checkpoint (ex.: "$\frac{8}{3\pi} \approx 0{,}85$") nem fórmula, nem PALAVRA que seja resposta de alguma lacuna/questão (ex.: lacuna pede "volume" → "volume" não aparece no checkpoint nem em outro subitem).
- **Dica sem resposta:** lembra pré-requisito ou analogia já vista; NÃO pode conter a expressão pedida nem um pedaço dela (ex.: aluno deve escrever $x = r\cos\theta$ → a dica não escreve a fórmula, aponta onde ela foi vista).
...
Para a implementação de **slides Reveal.js** (via loop-slides), use o agente **`implementador-slides`** (`.opencode/agent/implementador-slides.md`), que contém as regras específicas de slides. Este agente é exclusivo de exercícios de sala.
